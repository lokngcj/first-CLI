...
import type { RulesViolation, RuleSet, ViolationType } from '../types/index.js';
...
export function checkFileViolations(
  ...
  if (!isSourceFile(absolutePath)) return violations;
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (!isRequestCheckExempt && hasCode) {
      checkPattern(codeLine, VIOLATION_PATTERNS.directAxiosImport, () => {
        violations.push(makeViolation(relativePath, lineNum, line, 'direct_axios',
          'Direct axios import detected. Use the project unified request wrapper instead of importing axios directly.'));
      });

      checkPattern(codeLine, VIOLATION_PATTERNS.directAxiosRequire, () => {
        violations.push(makeViolation(relativePath, lineNum, line, 'direct_axios',
          'Direct axios require detected. Use the project unified request wrapper instead.'));
      });

      checkPattern(codeLine, VIOLATION_PATTERNS.directAxiosCall, () => {
        violations.push(makeViolation(relativePath, lineNum, line, 'direct_axios',
          'Direct axios method call detected. Replace with the project unified request wrapper.'));
      });

      checkPattern(codeLine, VIOLATION_PATTERNS.directFetch, () => {
        violations.push(makeViolation(relativePath, lineNum, line, 'direct_fetch',
          'Direct fetch() call detected. Use the project unified request wrapper instead.'));
      });

      checkPattern(codeLine, VIOLATION_PATTERNS.nonstandardHttpImport, () => {
        violations.push(makeViolation(relativePath, lineNum, line, 'nonstandard_api_call',
          'Non-standard HTTP client import detected. Use the project unified request wrapper instead.'));
      });

      checkPattern(codeLine, VIOLATION_PATTERNS.xmlHttpRequest, () => {
        violations.push(makeViolation(relativePath, lineNum, line, 'nonstandard_api_call',
          'XMLHttpRequest usage detected. Use the project unified request wrapper instead.'));
      });
    }
    ...
    while ((cmatch = chineseRe.exec(codeLine)) !== null) {
      if (hasStringContext(codeLine, cmatch.index)) {
        violations.push(makeViolation(relativePath, lineNum, line, 'i18n_hardcoding',
          'Hardcoded Chinese text detected. Replace with i18n translation function call (e.g., t("key") or i18n.t("key")).'));
        break;
      }
    }
    ...
    if (hasCode) {
      checkPattern(codeLine, VIOLATION_PATTERNS.useCallback, () => {
        violations.push(makeViolation(relativePath, lineNum, line, 'useCallback_abuse',
          'useCallback usage detected. Verify that memoization is necessary; wrapping stable functions in useCallback adds overhead without benefit.',
          'warning'));
      });
    }
  }
...
function hasStringContext(line: string, pos: number): boolean {
