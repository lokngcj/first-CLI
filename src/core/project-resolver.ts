 ...
 */
export function validateProject(projectPath: string): string {
  if (!isAbsolute(projectPath)) {
    throw new Error(
      `Project path must be absolute: ${projectPath}\n` +
  ...
  if (!isDirectory(appsDir)) {
    throw new Error(
      `Not a valid Monorepo project: no "${APPS_DIR}/" directory found under ${projectPath}\n` +
 ...
 */
export function resolveApp(projectRoot: string, targetFile: string): AppContext {
  ...
  if (!normalized.startsWith(`${APPS_DIR}/`)) {
    throw new Error(
      `Target file is not within an app under ${APPS_DIR}/: ${normalized}\n` +
  ...
  if (!isDirectory(appPath)) {
    throw new Error(
      `App directory does not exist: ${appPath}\n` +
  ...
  if (!fileExists(targetFileAbsolute)) {
    throw new Error(
      `Target file does not exist: ${targetFileAbsolute}\n` +
 ...
 */
export function resolveAppLax(projectRoot: string, targetFile: string): AppContext {
  let relativePath: string;
  if (isAbsolute(targetFile)) {
    relativePath = getRelativePath(targetFile, projectRoot);
  } else {
...
export function listApps(projectRoot: string): string[] {
