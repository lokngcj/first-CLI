 ...
 */
export function formatOutput(data: CommandOutput, format: OutputFormat): string {
  switch (format) {
    case 'json':
...
function formatText(data: CommandOutput): string {
  switch (data.command) {
...
function formatPreflightText(data: PreflightOutput): string {
  const lines: string[] = [];
...
function formatVerifyText(data: VerifyOutput): string {
  const lines: string[] = [];
  lines.push('═'.repeat(50));
...
function formatFixSuggestionsText(data: FixSuggestionsOutput): string {
  const lines: string[] = [];
  lines.push('═'.repeat(50));
...
function formatRulesCheckText(data: RulesCheckOutput): string {
  const lines: string[] = [];
  lines.push('═'.repeat(50));
...
function formatFigmaCompressText(data: FigmaCompressOutput): string {
  const lines: string[] = [];
  lines.push('═'.repeat(50));
...
function formatMarkdown(data: CommandOutput): string {
  switch (data.command) {
...
function formatPreflightMarkdown(data: PreflightOutput): string {
  const lines: string[] = [];
  lines.push('# Preflight — Context Overview');
  lines.push('');
...
function formatVerifyMarkdown(data: VerifyOutput): string {
  const lines: string[] = [];
  lines.push('# Verify — Code & Rules Check');
  lines.push('');
  lines.push(`| Field | Value |`);
...
function formatFixSuggestionsMarkdown(data: FixSuggestionsOutput): string {
  const lines: string[] = [];
  lines.push('# Fix Suggestions');
  lines.push('');
  lines.push(`| Field | Value |`);
...
function formatRulesCheckMarkdown(data: RulesCheckOutput): string {
  const lines: string[] = [];
  lines.push('# Rules Check — Project Health');
  lines.push('');
  lines.push(`| Field | Value |`);
...
function formatFigmaCompressMarkdown(data: FigmaCompressOutput): string {
  const lines: string[] = [];
  lines.push('# Figma Compress');
  lines.push('');
...
function formatExceptionsText(exceptions: RulesException[], indent = ''): string {
  const lines: string[] = [];
...
function formatViolationsText(violations: RulesViolation[]): string {
  const lines: string[] = [];
...
function formatQualityText(quality: QualityResult): string {
  const lines: string[] = [];
...
function formatExceptionsMarkdown(exceptions: RulesException[]): string {
  const lines: string[] = [];
  for (const exc of exceptions) {
...
function formatViolationsMarkdown(violations: RulesViolation[]): string {
  const lines: string[] = [];
