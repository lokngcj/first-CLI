...
import type { OutputFormat, VerifyOutput, RulesViolation } from '../types/index.js';
...
export async function verifyCommand(args: VerifyArgs): Promise<void> {
  ...
  let filesByApp: Map<string, { appPath: string; files: string[] }>;
  ...
  // 3. Process each app
  const allExceptions: VerifyOutput['exceptions'] = [];
  const allViolations: VerifyOutput['violations'] = [];
 ...
 */
function groupFilesByApp(
  projectRoot: string,
  filePaths: string[],
): Map<string, { appPath: string; files: string[] }> {
  const map = new Map<string, { appPath: string; files: string[] }>();
