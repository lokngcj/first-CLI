...
export async function fixSuggestionsCommand(args: FixSuggestionsArgs): Promise<void> {
  // 1. Validate project
  ...
  if (hasErrors(exceptions)) {
    const output: FixSuggestionsOutput = {
      schemaVersion: '1.0',
...
function getFixDescription(v: RulesViolation): string {
  switch (v.type) {
    case 'direct_axios':
...
function getFixSnippet(v: RulesViolation): string {
  switch (v.type) {
    case 'direct_axios':
...
function getChecklistItem(v: RulesViolation): string {
  switch (v.type) {
    case 'direct_axios':
